...
class Sweep {
 public:
  inline static std::vector<Node2D> sortPointsToLine(const Polygon2D& poly,
                                                     const Node2D& dir) {
    // dir 与扫描线同向，dir为单位向量
  ...
  inline static bool computeSweep(const Polygon2D& poly, const double offset,
                                  const Node2D& dir,
