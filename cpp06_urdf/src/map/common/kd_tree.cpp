...
void KDTree::initTreeNode(const std::shared_ptr<KDTreeNode>& root) {
...
void KDTree::build(std::vector<Node2D>& point_cloud) {
  if (root_ != nullptr) {
...
void KDTree::buildTree(std::shared_ptr<KDTreeNode>* root, int l, int r,
...
void KDTree::runOperation(std::shared_ptr<KDTreeNode>& root,
...
void KDTree::update(const std::shared_ptr<KDTreeNode>& root) {
...
int KDTree::deleteByRange(std::shared_ptr<KDTreeNode>* root,
...
void KDTree::deleteByPoint(std::shared_ptr<KDTreeNode>* root,
                           const Node2D& point) {
  if ((*root) == nullptr || (*root)->tree_deleted) {
    return;
  }
...
void KDTree::addByPoint(std::shared_ptr<KDTreeNode>* root, const Node2D& point,
                        int father_axis) {
  if (*root == nullptr) {
    *root = std::make_shared<KDTreeNode>();
    initTreeNode(*root);
...
void KDTree::addByRange(std::shared_ptr<KDTreeNode>* root,
                        BoxPointType boxpoint) {
  if ((*root) == nullptr) {
    return;
  }
...
void KDTree::search(const std::shared_ptr<KDTreeNode>& root, int k_nearest,
...
void KDTree::searchByRange(const std::shared_ptr<KDTreeNode>& root,
...
void KDTree::searchByRadius(const std::shared_ptr<KDTreeNode>& root,
...
void KDTree::deleteTreeNodes(std::shared_ptr<KDTreeNode>* root) {
  if (*root == nullptr) return;
...
bool KDTree::samePoint(const Node2D& a, const Node2D& b) {
  return (fabs(a.x - b.x) < epsilon && fabs(a.y - b.y) < epsilon);
}
double KDTree::calcDist(const Node2D& a, const Node2D& b) {
  return (a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y);
}
double KDTree::calcBoxDist(const std::shared_ptr<KDTreeNode>& node,
                           const Node2D& point) {
...
void KDTree::nearestSearch(const Node2D& point, int k_nearest,
...
void KDTree::boxSearch(const BoxPointType& box_of_point,
                       std::vector<Node2D>& result) {
  result.clear();
  searchByRange(root_, box_of_point, result);
  return;
}
void KDTree::radiusSearch(const Node2D& point, const double radius,
                          std::vector<Node2D>& result) {
  result.clear();
  searchByRadius(root_, point, radius, result);
  return;
}
void KDTree::addPoints(const std::vector<Node2D>& point_to_add) {
  for (size_t i = 0; i < point_to_add.size(); i++) {
    addByPoint(&root_, point_to_add[i], root_->division_axis);
  }
  return;
}
void KDTree::addPointBoxes(std::vector<BoxPointType>& box_points) {
  for (size_t i = 0; i < box_points.size(); i++) {
    addByRange(&root_, box_points[i]);
  }
  return;
}
void KDTree::deletePoints(const std::vector<Node2D>& point_to_del) {
  for (size_t i = 0; i < point_to_del.size(); i++) {
    deleteByPoint(&root_, point_to_del[i]);
  }
  return;
}
int KDTree::deletePointBoxes(std::vector<BoxPointType>& box_points) {
  int tmp_counter = 0;
  for (size_t i = 0; i < box_points.size(); i++) {
...
void KDTree::pushDown(const std::shared_ptr<KDTreeNode>& root) {
  if (root == nullptr) {
    return;
  }
...
void KDTree::flatten(const std::shared_ptr<KDTreeNode>& root,
                     std::vector<Node2D>& result) {
  if (root == nullptr) return;
  pushDown(root);
  if (!root->point_deleted) {
