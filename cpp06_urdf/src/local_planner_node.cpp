...
{
    local_path_length_ = this->declare_parameter<double>("local_path_length", 2.0);
    lookahead_distance_ = this->declare_parameter<double>("lookahead_distance", 0.6);
    ...
    RCLCPP_INFO(this->get_logger(), "Local planner started. lookahead=%.2f, max_v=%.2f, max_w=%.2f",
...
void LocalPlannerNode::globalPathCallback(const nav_msgs::msg::Path::SharedPtr msg)
{
    ...
    RCLCPP_INFO(this->get_logger(), "Received global path with %zu poses.", msg->poses.size());
}
...
size_t LocalPlannerNode::findNearestIndex(const nav_msgs::msg::Path& path, double x, double y) const
{
    size_t nearest = 0;
...
nav_msgs::msg::Path LocalPlannerNode::smoothPath(const nav_msgs::msg::Path& path)
{
    if (path.poses.size() < 3) return path;

    nav_msgs::msg::Path smoothed;
    smoothed.header = path.header;
    smoothed.poses = path.poses;

    for (int iter = 0; iter < 2; ++iter) {
        std::vector<geometry_msgs::msg::PoseStamped> temp = smoothed.poses;
        for (size_t i = 1; i + 1 < smoothed.poses.size(); ++i) {
            temp[i].pose.position.x = (smoothed.poses[i-1].pose.position.x
                                     + smoothed.poses[i].pose.position.x
                                     + smoothed.poses[i+1].pose.position.x) / 3.0;
            temp[i].pose.position.y = (smoothed.poses[i-1].pose.position.y
                                     + smoothed.poses[i].pose.position.y
                                     + smoothed.poses[i+1].pose.position.y) / 3.0;
        }
        smoothed.poses = std::move(temp);
    }
    return smoothed;
}
...
void LocalPlannerNode::planTimerCallback()
{
    ...
    double dist_to_goal = std::hypot(goal_pose.position.x - current_x_, goal_pose.position.y - current_y_);
    ...
    nav_msgs::msg::Path smoothed_local = smoothPath(local_path);

    // 计算每个点的朝向并发布局部路径
    for (size_t i = 0; i < smoothed_local.poses.size(); ++i) {
        double yaw = 0.0;
        if (i < smoothed_local.poses.size() - 1) {
            double dx = smoothed_local.poses[i+1].pose.position.x - smoothed_local.poses[i].pose.position.x;
            double dy = smoothed_local.poses[i+1].pose.position.y - smoothed_local.poses[i].pose.position.y;
            yaw = std::atan2(dy, dx);
        } else if (i > 0) {
            double dx = smoothed_local.poses[i].pose.position.x - smoothed_local.poses[i-1].pose.position.x;
            double dy = smoothed_local.poses[i].pose.position.y - smoothed_local.poses[i-1].pose.position.y;
            yaw = std::atan2(dy, dx);
        }
    ...
    if (target_idx == nearest_idx && nearest_idx + 1 < global_path_.poses.size()) {
...
double LocalPlannerNode::yawFromQuaternion(const geometry_msgs::msg::Quaternion &q)
{
