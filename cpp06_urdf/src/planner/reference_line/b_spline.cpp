...
std::vector<double> BSplineReferenceLine::buildKNot(const int n, const int p,
                                                    const int type) {
...
double BSplineReferenceLine::bsplineBaseFunc(const int i, const int p,
  ...
  if (p == 0) {
    ...
    double result = 0.0;
    result += (knots[i + p] - knots[i] > EPSILON)
                  ? (t - knots[i]) / (knots[i + p] - knots[i]) *
                        bsplineBaseFunc(i, p - 1, knots, t)
                  : 0.0;
    result += (knots[i + p + 1] - knots[i + 1] > EPSILON)
                  ? (knots[i + p + 1] - t) / (knots[i + p + 1] - knots[i + 1]) *
                        bsplineBaseFunc(i + 1, p - 1, knots, t)
                  : 0.0;
    return result;
  }
  return 0.0;
...
nav_msgs::msg::Path BSplineReferenceLine::smoothPath(
  ...
  for (double i = 0.0; i <= 1.0; i += interval_) {
    ...
    smoothed_pose.pose.position.z =
        path.poses[0].pose.position.z;  // 保持原始高度
